...
export async function sendPasswordResetEmail(email: string, resetToken: string): Promise<boolean> {
  if (!sesClient) {
    console.log(`[DEBUG] AWS SES não configurado. Link de recuperação: ${env.FRONTEND_URL}/auth/reset-password?token=${resetToken}`);
    return true;
  }
  ...
  try {
    await sesClient.send(
      new SendEmailCommand({
        Source: env.AWS_SES_FROM_EMAIL || "noreply@cantinaonline.com",
        Destination: {
          ToAddresses: [email],
        },
        Message: {
          Subject: {
            Data: "Recupere sua senha - Cantina Online",
            Charset: "UTF-8",
          },
          Body: {
            Html: {
    ...
    console.log(`[INFO] E-mail de recuperação enviado para ${email} via AWS SES`);
    return true;
  } catch (error) {
    console.error("Erro ao enviar e-mail via AWS SES:", error);
    return false;
  }
}
